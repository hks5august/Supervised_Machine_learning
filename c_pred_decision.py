...
models.append(('svc-rbf_w',SVC(kernel="rbf",gamma=0.01,C=2,probability=True,class_weight="balanced")))
...
for name, model in models:
	...
	cv_predict_results1 = model_selection.cross_val_predict(model, X, y, cv=kfold,method='decision_function')
	y_pred_prob=[i[1] for i in cv_predict_results1]
	df[i]=y_pred_prob
	names.append(name)
	model.fit(X,y)
	pred_test1=model.decision_function(X_test)
	df.rename(columns ={i: name}, inplace =True)
	#y_pred_prob_test=[i[1] for i in pred_test1]
	dft[i]=pred_test1
	dft.rename(columns ={i: name}, inplace =True)
	i += 1
df.to_csv("train_rna_pred_d1")
dft.to_csv("test_rna_pred_d1") 
